...
export async function getChaptersInfo() {
  try {
    ...
    console.error("Failed to load chapters info:", err)
    return null
  }
}
...
export async function getChapterDetails(chapterNo, translation = null) {
  ...
  try {
    // Try main API first (alquranarabia.com)
    ...
    console.log(`Main API failed for translation ${trans}, trying QuranCDN API...`)
  }
  
  // Fallback to QuranCDN API for translations not available in main API
  try {
    const quranCdnUrl = `${config.quranCdnApiBaseUrl}/translations/${trans}/chapters/${chapterNo}`
    console.log(`Trying QuranCDN API: ${quranCdnUrl}`)
    
    const data = await enhancedFetch(quranCdnUrl)
    
    if (data && data.verses) {
      console.log(`Successfully loaded ${trans} from QuranCDN API`)
      return data
    }
  } catch (error) {
    console.error(`Both APIs failed for translation ${trans} chapter ${chapterNo}:`, error.message)
  }
